RunalyzeLeaflet.Routes = (function($, RunalyzeLeaflet, Math){
	return function(parent) {
		...
		function positionTooltip(e) {
			var $e = $(tooltip).parent(),
				...
				parentOffset = parent.element().offset(),
				layerX = e.originalEvent.clientX - parentOffset.left + window.pageXOffset,
				layerY = e.originalEvent.clientY - parentOffset.top + window.pageYOffset;
		...
		self.addRoute = function(id, object) {
			...
			object.polyline = L.polyline( object.segments, object.options );
	...
	self.moveTimeMarker = function(sec) {
		var id = self.routeid;
		...
		for (var s = 0; s < objects[id].segmentsInfo.length; ++s) {
			...
			if (objects[id].segmentsInfo[s][0][1] < sec && sec < objects[id].segmentsInfo[s][segmentLength-1][1]) {
				counter = 0;
				lower = 0;
				index = 0;
				upper = segmentLength - 1;

				while ((upper - lower) > 1 && counter < maxBisectionIterations && Math.abs(objects[id].segmentsInfo[s][index][1] - sec) > epsilonSec) {
					counter++;

					index = Math.floor( (upper+lower)/2 );

					if (objects[id].segmentsInfo[s][index][1] > sec) {
						upper = index;
					} else {
						lower = index;
					}
				...
				if (lower != upper) {
					var ratio = (sec - objects[id].segmentsInfo[s][lower][1]) / (objects[id].segmentsInfo[s][upper][1] - objects[id].segmentsInfo[s][lower][1]);
					pos = [
						objects[id].segments[s][lower][0] + ratio * (objects[id].segments[s][upper][0] - objects[id].segments[s][lower][0]),
						objects[id].segments[s][lower][1] + ratio * (objects[id].segments[s][upper][1] - objects[id].segments[s][lower][1])
					];
