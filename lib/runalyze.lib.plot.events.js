...
RunalyzePlot.Events = (function($, parent){
			...
			if (mapId) {
				if ($(".training-row-plot").has($("#"+key)).length && !opt.series.bars.show) {
					if (plot.getOptions().xaxis.mode != 'time') {
						...
						moveMapMarker(mapId, distanceSeries.data[j][1]);
					}
				}
			}
	...
	function getTrainingMapId() {
		return $("#training-map").find('.map').attr('id');
	}

	function moveMapMarker(mapId, pos) {
		RunalyzeLeaflet.get(mapId).Routes.movePosMarker(pos);
	}
	
	function moveMapMarkerSec(sec) {
		if (RunalyzeLeaflet && RunalyzeLeaflet.Routes) {
			RunalyzeLeaflet.Routes.moveTimeMarker(sec);
		}
	}
	...
	function unsetMapMarker() {
		...
		if (mapId) {
			RunalyzeLeaflet.get(mapId).Routes.unsetPosMarker();
		}
	}
	...
	function show(key, pos, content, posClass, fixed) {
		if (pos.hasOwnProperty('x') && pos.hasOwnProperty('y')) {
			...
			if (fixed === true) {
                ...
                cssProperties['right'] = 'auto';
				cssProperties['position'] = 'absolute';

                if (tt.is(':visible')) {
                	if (tt.hasClass('left')) {
                        cssProperties['left'] -= (tt.width() - currentWidth);
					}
				...
				if (posClass == 'right' && pos.x > 0.5 * $plot.width()) {
                    cssProperties['right'] = $plot.width() - pos.x + 30;
                    cssProperties['left'] = 'auto';
                    posClass = 'left';
				...
				if (posClass == 'right') {
					if (pos.x > 0.5 * $(window).width()) {
						cssProperties['left'] = 'auto';
						cssProperties['right'] = $(window).width() - (pos.x - $(document).scrollLeft()) + 30;
						tt.removeClass('right');
