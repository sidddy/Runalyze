...
class AthletePosterCommand extends ContainerAwareCommand
{
    ...
    protected function createJsonFilesFor(Account $athlete, $sportid, InputInterface $input, OutputInterface $output)
    {
        ...
        $sportRepository = $this->getContainer()->get('doctrine')->getRepository('CoreBundle:Sport');
        $query = $trainingRepository->getQueryForJsonPosterData($athlete, $sportRepository->find($sportid), $input->getArgument('year'));
    ...
    protected function listSpecialFiles(Account $athlete, $sportid, InputInterface $input, OutputInterface $output)
    {
        ...
        $races = $raceRepository->findBySportAndYear($athlete, $sportRepository->find($sportid), $input->getArgument('year'));
