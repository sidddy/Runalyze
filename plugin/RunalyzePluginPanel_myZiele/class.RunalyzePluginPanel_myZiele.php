...
$PLUGINKEY = 'RunalyzePluginPanel_myZiele';

/**
 * Class: RunalyzePluginPanel_myZiele
 * @author Ulrich Kiermayr <ulrich@kiermayr.at>
 * License: GPL
 * @package Runalyze\Plugins\Panels
 */
class RunalyzePluginPanel_myZiele extends PluginPanel
{
	/**
	...
	protected function initConfiguration()
	{
		...
		foreach ($this->getTimeset() as $i => $timeset) {
			...
			$Goal = new PluginConfigurationValueInt('ziel_' . $i, sprintf(__('%s: goal km'), $timeset['name']), $GoalHint, 0);
			$GoalHM = new PluginConfigurationValueInt('ziel_hm_' . $i, sprintf(__('%s: goal HM'), $timeset['name']), $GoalHint, 0);

			$Configuration->addValue($ShowGoal);
			$Configuration->addValue($Goal);
			$Configuration->addValue($GoalHM);
		}
...
//		$(this).closest("li.with-submenu").find("span").text($(this).text())
	...
	protected function displayContent()
	{
		...
		foreach ($this->getTimeset() as $i => $timeset) {
			...
			$this->showTimeset($timeset, $this->Configuration()->value('ziel_' . $i), $this->Configuration()->value('ziel_hm_' . $i));
	 ...
	 * @param int $goal
	 */
	private function showTimeset(&$Timeset, $goal, $goal_hm)
	{
		$this->clearLines();
		...
		if ($dat['distanz_sum'] > 0) {
			$this->addHeadline(__('Current'), $dat['distanz_sum'], $dat['anzahl'], true, $dat['hm_sum']);
			if ($showDetails) {
				$this->addLine(__('avg.').' '.__('Day'), $days > 0 ? $dat['distanz_sum'] / $days : 0, 0, $days > 0 ? $dat['hm_sum'] / $days : 0);

				if ($days > 7)
					$this->addLine(__('avg.').' '.__('Week'), $days > 0 ? 7 * $dat['distanz_sum'] / $days : 0, $dat['anzahl'] / $days * 7, $days > 0 ? 7 * $dat['hm_sum'] / $days : 0);

				if ($days > 0)
					$this->addHeadline(__('Prognosis'), $dat['distanz_sum'] / $days * $dauer, $dat['anzahl'] / $days * $dauer, false, $dat['hm_sum'] / $days * $dauer);
			}
		}
		if ($goal > 0) {
			$this->addHeadline(__('Goal'), $goal, 0, false, $goal_hm);
			if ($dat['distanz_sum'] < $goal) {
				$togo = $goal - $dat['distanz_sum'];
				$togo_hm = $goal_hm - $dat['hm_sum'];
				$this->addLine(sprintf(__('%d days left'), $rest), $togo, 0, $togo_hm);
				if ($showDetails) {

					$this->addLine(__('avg.').' '.__('Day'), $rest > 0 ? $togo / $rest : 0, 0, $rest > 0 ? $togo_hm / $rest : 0);

					if ($rest > 7)
						$this->addLine(__('avg.').' '.__('Week'), 7 * $togo / $rest, 0, 7 * $togo_hm / $rest);
				}
			} else {
			...
			$diff = $goal / $dauer * $days - $dat['distanz_sum'];
		 	$diff_hm = $goal_hm / $dauer * $days - $dat['hm_sum'];

			if ($showDetails) {

				$this->addHeadline(__('Pace Bunny'), $goal / $dauer * $days, 0, false,  $goal_hm / $dauer * $days);
				$this->addLine(__('avg.').' '.__('Day'), $goal / $dauer, 0, $goal_hm / $dauer);
				...
				if ($diff_hm > 0)
					$this->addLine(__('Behind'), 0,0, abs($diff_hm)); 
				else
					$this->addLine(__('Ahead'), 0, 0, abs($diff_hm));

				$this->addHeadline(__('Head 2 Head'));
			}
			$this->addBar(__('Me').' (km)', $dat['distanz_sum'], $goal, $diff > 0 ? '#f99' : '#9f9');
			$this->addBar(__('Me').' (Hm)', $dat['hm_sum'], $goal_hm, $diff_hm > 0 ? '#f99' : '#9f9');
	 ...
	 */
	private
	function addHeadline($name, $distance = 0, $num = 0, $noTopBorder = false, $hm = 0)
	{
		$this->Lines[] = array(
			'name' => $name,
			'lvl' => 1,
	 ...
	 */
	private
	function addLine($name, $distance = 0, $num = 0, $hm = 0)
	{
		$this->Lines[] = array(
			'name' => $name,
			'lvl' => 2,
	...
	function showLine(&$Line)
	{
		...
		$Ele = isset($Line['hm']) && $Line['hm'] > 0 ? ' <span ' . ($Line['lvl'] == 1 ? $span_format : '') . '>' . Elevation::format($Line['hm'], false) . ' Hm</span>' : '';

		echo '<p' . $p_format . '>' .
			'<span class="right">' .
			$NumberOfActivities . ' ' . $Distance . $Ele .
			'</span>' .
	...
	function fetchDataSince($timestamp)
	{
		...
		')->fetch();
		if (!is_array($Data))
			return array('anzahl' => 0, 'distanz_sum' => 0, 'dauer_sum' => 0, 'hm_sum' => 0);
